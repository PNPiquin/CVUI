...
namespace ip {
std::shared_ptr<Matrix<uint8_t>> get_borders(std::shared_ptr<Matrix<uint8_t>> img, uint8_t threshold)
{
  ...
  for (size_t row = 0; row < img->get_rows(); ++row) {
    for (size_t col = 0; col < img->get_cols(); ++col) {
      if (is_border(img, row, col, threshold, 4)) {
        border_img->operator()(row, col) = 255;
      }
    }
    if (row % 100 == 0) {
      printf("Processed row (%lld/%d)\n", row, img->get_rows());
    }
  }
  return border_img;
}
